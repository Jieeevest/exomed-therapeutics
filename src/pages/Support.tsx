      ...
      <div className="pt-24 pb-20 px-4 min-h-screen relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative z-10">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
            <button
              onClick={() =>
                view !== "list" ? setView("list") : navigate("/app")
              }
            ...
            {view === "list" && (
              <button
                ...
                className="flex items-center gap-2 px-6 py-2.5 bg-gold-gradient text-white rounded-xl text-sm font-bold hover:opacity-90 transition-opacity shadow-[0_0_20px_rgba(78,130,129,0.2)]"
              >
          ...
          <AnimatePresence mode="wait">
              ...
              >
                <div className="mb-10">
                  ...
                  <p className="text-slate-400 text-lg">
                    Kelola laporan kendala dan pertanyaan Anda langsung dengan
                    tim kami.
                  </p>
                </div>
                ...
                {loading ? (
                  ...
                  <div className="py-24 text-center border border-white/5 rounded-[32px] bg-[#0a0a0a]">
                    ...
                    <p className="text-slate-500 max-w-sm mx-auto mb-8">
                      Apakah Anda mengalami kendala? Buat tiket laporan pertama
                      Anda sekarang.
                    </p>
                    <button
                      onClick={() => setView("create")}
                      className="px-8 py-3 bg-white/10 border border-white/20 text-white rounded-xl text-sm font-bold hover:bg-white/20 transition-colors"
                    >
                      Mulai Laporan Baru
                    </button>
                  </div>
                    ...
                    {tickets.map((t) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <div
                              className={`p-4 rounded-2xl border ${cfg.bg} ${cfg.border} ${cfg.color}`}
                            >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-3 mb-1">
                                <span
                                  className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full border ${cfg.bg} ${cfg.border} ${cfg.color}`}
                                >
                                  {cfg.label}
                                </span>
                              ...
                              <div className="flex items-center gap-3 text-xs text-slate-500 font-medium">
                                <span className="flex items-center gap-1.5">
                                  ...
                                  {parseInt(t.reply_count)} balasan
                                </span>
                                <span>&bull;</span>
                                <span className="flex items-center gap-1.5">
                                  <Clock className="w-3.5 h-3.5" /> Diperbarui{" "}
              ...
              >
                <div className="max-w-2xl mx-auto">
                  <div className="mb-10 text-center">
                    ...
                    <p className="text-slate-400 text-lg">
                      Ceritakan kendala Anda dengan jelas agar tim kami dapat
                    ...
                    <div className="space-y-6 relative z-10">
                      ...
                      <div className="pt-4 flex flex-col-reverse sm:flex-row gap-3">
                        <button
                          ...
                          className="px-6 py-3.5 bg-white/5 border border-white/10 rounded-xl text-sm font-bold hover:bg-white/10 transition-colors sm:w-1/3"
                        >
                          Batal
                        </button>
                        <button
                          type="submit"
                          disabled={creating}
                          className="px-6 py-3.5 bg-gold-gradient text-white font-bold rounded-xl text-sm hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2 sm:w-2/3 shadow-[0_0_20px_rgba(78,130,129,0.2)]"
                        >
            ...
            {view === "detail" && selected && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                className="max-w-4xl mx-auto flex flex-col h-[80vh] md:h-[70vh] bg-[#0a0a0a] border border-white/10 rounded-[32px] overflow-hidden shadow-2xl relative"
              >
                {/* Chat Header */}
                <div className="px-6 py-5 border-b border-white/10 bg-black/40 backdrop-blur-md flex items-center justify-between shrink-0 z-10">
                  <div className="flex-1 min-w-0 pr-4">
                    ...
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-widest mt-1">
                      ...
                      &bull; TIKET #{selected.id.split("-")[0].toUpperCase()}
                    </p>
                  </div>
                  {(() => {
                    ...
                    return (
                      <div
                        className={`hidden sm:flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full border ${cfg?.bg} ${cfg?.border} ${cfg?.color}`}
                      >
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 relative">
                  ...
                  {(selected.replies || []).map((r: any) => (
                    <div
                      key={r.id}
                      className={`flex gap-3 max-w-[85%] ${r.is_admin ? "mr-auto" : "ml-auto flex-row-reverse"}`}
                    >
                      <div
                        className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-black shrink-0 border ${r.is_admin ? "bg-primary/20 text-primary border-primary/30" : "bg-white/10 text-white border-white/20"}`}
                      >
                        {r.is_admin ? "CS" : r.username[0].toUpperCase()}
                      </div>
                      <div
                        className={`rounded-3xl px-5 py-4 text-[15px] ${r.is_admin ? "bg-white/5 border border-white/10 rounded-tl-sm" : "bg-primary text-white font-medium rounded-tr-sm shadow-[0_5px_15px_rgba(56,189,248,0.2)]"}`}
                      >
                        <div
                          className={`text-xs font-black mb-1.5 uppercase tracking-widest ${r.is_admin ? "text-primary" : "text-black/60"}`}
                        >
                          {r.is_admin ? "Tim Support" : r.username}
                        </div>
                        <p
                          className={`whitespace-pre-wrap leading-relaxed ${r.is_admin ? "text-slate-300" : "text-black/90"}`}
                        >
                          {r.message}
                        </p>
                        <div
                          className={`text-[10px] font-bold mt-2 text-right ${r.is_admin ? "text-slate-600" : "text-black/50"}`}
                        >
                ...
                <div className="p-4 border-t border-white/10 bg-black/40 backdrop-blur-md shrink-0">
                  {selected.status !== "closed" ? (
                    <form
                      ...
                      className="relative flex items-end gap-3 bg-[#030303] border border-white/10 rounded-2xl p-2 focus-within:border-primary/50 transition-colors"
                    >
                      <textarea
                        rows={1}
                        value={replyMsg}
                      ...
                      <button
                        ...
                        className="w-11 h-11 bg-primary text-white rounded-xl flex items-center justify-center hover:bg-primary/90 disabled:opacity-40 disabled:hover:bg-primary transition-all shrink-0"
                      >
